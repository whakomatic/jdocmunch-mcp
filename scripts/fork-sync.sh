...
#   fork-sync pr <branch> <commit|PR-X>... [--dry-run]
#                                    build a PR branch from those commits and
#                                    open the PR
#   fork-sync mark <commit> <status> set a commit's Fork-Status trailer
#
...
#                     A parked PR keeps no branch: its commit on local/main
#                     is the record, and `pr` rebuilds the branch on demand.
#
...
# FORK-STATUS
#
# Every commit on local/main carries one `Fork-Status:` trailer saying what it
# is for upstream:
#
#   local          ours only; never meant for upstream
#   parked PR-X    PR prepared (docs/prs/PR-X-*.md) but not submitted
#   pr #N          submitted as upstream PR #N
#   issue #N       upstream issue #N covers it (a proposal, a report)
#
# `status` prints it. After a replay, `update` asks GitHub about every `pr #N`
# and `issue #N`: a merged PR or closed issue whose commit is still here means
# upstream took the change in another form (a squash, a reimplementation),
# which git cherry cannot see. `pr` strips the trailer from the PR's commits.
#
...
# The machine's own JDOCMUNCH_*/FASTEMBED_* settings (the proxy's embedding
# config) are unset: tests assume defaults, and e.g. JDOCMUNCH_EMBED_CHARS
# changes every embed-cache key they compare.
failed_ids() {
    local dir=$1 v unset=(); shift
    for v in $(compgen -e | grep -E '^(JDOCMUNCH_|FASTEMBED_)' || true); do unset+=(-u "$v"); done
    (cd "$dir" && env "${unset[@]}" PYTHONPATH=src python -m pytest -q -rfE -p no:cacheprovider "$@" </dev/null 2>&1 || true) \
...
restart_proxy_if_up() {
    ...
    [ -x "$p" ] || return 0
    # Captured, not piped: under pipefail, `status | grep -q` fails whenever
    # grep exits on the first line and status dies of SIGPIPE writing the rest.
    local out
    out=$("$p" status </dev/null 2>/dev/null || true)
...
# Our commits as "<sha> <Fork-Status>" lines, oldest first. The status is
# empty when the trailer is missing.
fork_statuses() {
    git log --reverse --format='%H %(trailers:key=Fork-Status,valueonly,separator=%x2C)' "$UPSTREAM..$MAIN"
}

list_ours() {
    local c st
    while read -r c st; do
        printf '    %s %-14s %s\n' "$(git rev-parse --short "$c")" "[${st:-NO STATUS}]" \
            "$(git log -1 --format=%s "$c")"
    done < <(fork_statuses)
}

# Commits whose linked upstream PR merged or issue closed, yet still on
# local/main: upstream took the change in a form git cherry cannot match.
# Returns 1 when any are found, so update can say so; it never blocks.
check_upstream_links() {
    command -v gh >/dev/null 2>&1 || { echo "    (gh not installed; skipped)"; return 0; }
    local c st kind num state found=0
    while read -r c st; do
        case "$st" in
            "pr #"*)    kind=pr ;;
            "issue #"*) kind=issue ;;
        ...
        num=${st##*#}
        state=$(GITHUB_TOKEN="" gh "$kind" view "$num" --repo "$UPSTREAM_REPO" \
            --json state --jq .state 2>/dev/null) || state="UNKNOWN"
        case "$state" in
            MERGED|CLOSED)
                found=1
                printf '    %s %s is %s upstream: check whether upstream now does this, then drop it or mark it local\n' \
                    "$(git log -1 --format='%h %s' "$c")" "$st" "$state" ;;
            UNKNOWN) echo "    $(git rev-parse --short "$c") $st: gh query failed" ;;
        esac
    done < <(fork_statuses)
...
cmd_status() {
    ...
    echo
    echo "linked upstream PRs/issues that are closed:"
    check_upstream_links || true
...
finish_update() {
    ...
    echo "==> Commits whose upstream PR merged or issue closed, but still here"
    check_upstream_links || true
...
cmd_pr() {
    ...
    [ -n "$branch" ] && [ ${#commits[@]} -gt 0 ] \
        || die "usage: fork-sync pr <branch> <commit|PR-X>... [--dry-run]"
    ! git rev-parse --verify --quiet "refs/heads/$branch" >/dev/null \
        || die "Branch $branch already exists."

    git fetch "$UPSTREAM_REMOTE" --quiet

    # A PR-X argument names the commits marked `parked PR-X`.
    local resolved=() c st
    for a in "${commits[@]}"; do
        case "$a" in
            PR-*)
                local n=${#resolved[@]}
                while read -r c st; do
                    [ "$st" = "parked $a" ] && resolved+=("$c")
                done < <(fork_statuses)
                [ ${#resolved[@]} -gt "$n" ] || die "No commit on $MAIN is marked 'parked $a'." ;;
            *) resolved+=("$(git rev-parse --verify "$a^{commit}")") || die "Not a commit: $a" ;;
        esac
    done
    commits=("${resolved[@]}")

    # Build the branch by ref updates alone, through a temporary index, so the
    # working tree is never touched and the running install keeps serving
    # local/main throughout. A 3-way apply carries a commit over context that
    # our other commits changed (a shared import block, say); a real conflict
    # stops with nothing changed.
    local tip idx
    tip=$(git rev-parse "$UPSTREAM")
    # Inside the git dir, not mktemp: under MSYS_NO_PATHCONV a /tmp path
    # reaches git.exe unconverted and it cannot open it.
    idx="$(git rev-parse --git-dir)/fork-sync-pr.index"
    CLEANUP+=("$idx")
    for c in "${commits[@]}"; do
        rm -f "$idx"
        GIT_INDEX_FILE=$idx git read-tree "$tip"
        if ! git diff --binary "$c^" "$c" | GIT_INDEX_FILE=$idx git apply --cached --3way >/dev/null 2>&1; then
            die "$(git log -1 --format='%h %s' "$c") conflicts with $UPSTREAM in:
$(GIT_INDEX_FILE=$idx git ls-files -u | cut -f2 | sort -u | sed 's/^/    /')
It depends on another commit of ours (include that one too), or needs a hand
resolution in a scratch checkout. Nothing was changed."
        fi
        # The PR carries the commit's message and author, minus our trailer.
        tip=$(git log -1 --format=%B "$c" | grep -v '^Fork-Status:' | git stripspace \
            | GIT_AUTHOR_NAME=$(git log -1 --format=%an "$c") \
              GIT_AUTHOR_EMAIL=$(git log -1 --format=%ae "$c") \
              GIT_AUTHOR_DATE=$(git log -1 --format=%aD "$c") \
              git commit-tree "$(GIT_INDEX_FILE=$idx git write-tree)" -p "$tip")
    done
    git branch --no-track "$branch" "$tip"
    ...
    echo "==> Record it on $MAIN, once per commit:"
    for c in "${commits[@]}"; do
        echo "    fork-sync mark $(git rev-parse --short "$c") 'pr #<number>'"
    done
}

# Rewrite one commit's Fork-Status on local/main. Only messages change, so the
# replay leaves every file as it was.
cmd_mark() {
    local target=${1:-} st=${2:-}
    [ -n "$target" ] && [ -n "$st" ] || die "usage: fork-sync mark <commit> <status>"
    case "$st" in
        local|"parked PR-"?*|"pr #"[0-9]*|"issue #"[0-9]*) ;;
        *) die "Status must be one of: local, 'parked PR-X', 'pr #N', 'issue #N'." ;;
    esac
    [ "$(git symbolic-ref --quiet --short HEAD || true)" = "$MAIN" ] \
        || die "Run this from the checkout that has $MAIN checked out."
    require_clean
    target=$(git rev-parse --verify "$target^{commit}") || die "Not a commit: $target"
    git merge-base --is-ancestor "$UPSTREAM" "$target" && [ "$target" != "$(git rev-parse "$UPSTREAM")" ] \
        && git merge-base --is-ancestor "$target" "$MAIN" \
        || die "$target is not one of our commits on $MAIN."

    local tree_before
    tree_before=$(git rev-parse "$MAIN^{tree}")
    # --trailer with ifExists=replace swaps the old value for the new one.
    GIT_SEQUENCE_EDITOR="sed -i '/^pick $(git rev-parse --short "$target")/a exec git -c trailer.ifexists=replace commit -q --amend --no-edit --trailer \"Fork-Status: $st\"'" \
        git rebase -q -i "$UPSTREAM" </dev/null \
        || die "Rewording stopped; 'git rebase --abort' puts everything back."
    [ "$(git rev-parse "$MAIN^{tree}")" = "$tree_before" ] \
        || die "The tree changed while rewording, which should be impossible. Compare with ORIG_HEAD."
    local subject
    subject=$(git log -1 --format=%s "$target")
    list_ours | grep -F "[$st]" | grep -F "$subject" \
        || die "No commit '$subject' is marked '$st' after the replay; check 'fork-sync status'."
}
...
case "${1:-}" in
    ...
    pr)     shift; cmd_pr "$@" ;;
    mark)   shift; cmd_mark "$@" ;;
    *)      die "usage: fork-sync status | update [--no-push] | pr <branch> <commit|PR-X>... [--dry-run] | mark <commit> <status>" ;;
esac
